/*
     ...
     |  ___/ | '_ \| |/ /  \___ \| '_ \| | '_ \ / _` |
     | |   | | | | |   <   ____) | | | | | |_) | (_| |
     |_|   |_|_| |_|_|\_\ |_____/|_| |_|_|_.__/ \__,_|
    
    
    Pink Shiba - $PSHIB

    12% total tax (variable higher tax on sells, starting at 24% at launch and lowering during the first few days)
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3); // DAI
...
contract PinkShiba is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // sells have a higher fee (200% at launch, will be reduced over the first few days)
    ...
    constructor() ERC20("Pink Shiba", "PSHIB") {
    	...
    	teamPayWallet = address(owner());
    	developmentWallet = address(owner());
    	buyBackWallet = address(owner());
        liquidityWallet = address(owner()); // leave as owner to start, can switch to a dead address later if desired.
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 25 / 10000)/1e18, "Cannot set maxTransactionAmount lower than initial");
        maxTransactionAmount = newNum * (10**18);
    }
    
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 1 / 100)/1e18, "Cannot set maxWallet lower than initial");
    ...
    function updateSellPenalty(uint256 sellFactor) external onlyOwner {
        require(sellFactor >= 100 && sellFactor <= 200, "sellFactor must be between 100 and 200");
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 devAmt = teamPayAmt / 5; // 20% of team amount goes to contract dev
        
        teamPayAmt -= devAmt; // subtract dev and team amount from marketing ammount.
